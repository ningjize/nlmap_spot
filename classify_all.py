...
if cache_images:
    if cache_img_exists:
        img2vectorvild_dir = pickle.load(
            open(cache_path+img_dir_name+"_images_vild", "rb"))
        img2vectorclip_dir = pickle.load(
            open(cache_path+img_dir_name+"_images_clip", "rb"))
    else:
        img2vectorvild_dir = {}
        img2vectorclip_dir = {}
...
for img_name in tqdm(img_names):
    ...
    if cache_images and cache_img_exists:
        image, image_height, image_width, valid_indices, detection_roi_scores, detection_boxes, detection_masks, detection_visual_feat, rescaled_detection_boxes = img2vectorvild_dir[
            img_name]
    else:
        image, image_height, image_width, valid_indices, detection_roi_scores, detection_boxes, detection_masks, detection_visual_feat, rescaled_detection_boxes = extract_roi_vild(
            image_path, session, overall_fig_size, params)
    if cache_images and not cache_img_exists:
        img2vectorvild_dir[img_name] = [image, image_height, image_width, valid_indices, detection_roi_scores,
                                        detection_boxes, detection_masks, detection_visual_feat, rescaled_detection_boxes]
    ...
    # np.array([i for i in indices if np.argmax(scores_all[i]) != 0])
    indices_fg = indices
    ...
    for anno_idx in indices[0:int(n_boxes)]:
        ...
        y1, x1, y2, x2 = int(np.floor(bbox[0])), int(
            np.floor(bbox[1])), int(np.ceil(bbox[2])), int(np.ceil(bbox[3]))
        ...
        clip_image_features = clip_image_features / \
            clip_image_features.norm(dim=1, keepdim=True)
        ...
        img_w_mask = plot_mask(
            mask_color, alpha, raw_image, segmentations[anno_idx])
        crop_w_mask = img_w_mask[y1:y2, x1:x2, :]

        if not headless and vis_details:
            fig, axs = plt.subplots(1, 4, figsize=(fig_size_w, fig_size_h), gridspec_kw={
                                    'width_ratios': [3, 1, 1, 2]}, constrained_layout=True)

            # Draw bounding box.
            rect = patches.Rectangle(
                (x1, y1), x2-x1, y2-y1, linewidth=line_thickness, edgecolor='r', facecolor='none')
            ...
            fontsize = max(
                min(fig_size_h / float(len(category_names)) * 45, 20), 8)
    ...
    # plt.show()
if cache_images and not cache_img_exists:
    pickle.dump(img2vectorvild_dir, open(
        cache_path+img_dir_name+"_images_vild", "wb"))
    pickle.dump(img2vectorclip_dir, open(
        cache_path+img_dir_name+"_images_clip", "wb"))
