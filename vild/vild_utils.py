...
def display_image(path_or_array, size=(10, 10)):
    if isinstance(path_or_array, str):
        image = np.asarray(Image.open(
            open(path_or_array, 'rb')).convert("RGB"))
    else:
        image = path_or_array
...
def extract_roi_vild(image_path, session, overall_fig_size, params):
    ...
    return(image, image_height, image_width, valid_indices, detection_roi_scores, detection_boxes, detection_masks, detection_visual_feat, rescaled_detection_boxes)


def vild_main(image_path, category_name_string, params, temperature=100.0, use_softmax=False):
    #################################################################
    # Hyperparameters for drawing
    configire_matplotlib_settings()
    line_thickness = 2
    fig_size_w = 35
    ...
    image, image_height, image_width, valid_indices, detection_roi_scores, detection_boxes, detection_masks, detection_visual_feat, rescaled_detection_boxes = extract_roi_vild(
        image_path, overall_fig_size, params)
    #################################################################
...
if __name__ == "__main__":
    ...
    numbered_categories = [{'name': str(idx), 'id': idx, }
                           for idx in range(50)]
    ...
    vild_main(image_path, category_name_string, params)
